...
def hamil_cons(t,t1,i_x,f_x,i_y,f_y,i_z,f_z,U,basis,N):
    
    hop_right=[[t,i_x[i],f_x[i]] for i in range(len(i_x))] + [[t,i_y[j],f_y[j]] for j in range(len(i_y))]+ [[t,i_z[m],f_z[m]] for m in range(len(i_z))]
    hop_left =[[-t,i_x[i],f_x[i]] for i in range(len(i_x))] + [[-t,i_y[j],f_y[j]] for j in range(len(i_y))] + [[-t,i_z[m],f_z[m]] for m in range(len(i_z))]
    ...
    hop_right_z_polarized_1 =[[t1,i_z[m],f_z[m]] for m in range(len(i_z))]
    hop_right_z_polarized_2 =[[-t1,i_z[m],f_z[m]] for m in range(len(i_z))]
    hop_left_z_polarized_1 =[[-t1,i_z[m],f_z[m]] for m in range(len(i_z))]
    hop_left_z_polarized_2 =[[t1,i_z[m],f_z[m]] for m in range(len(i_z))]
    ...
    static= [	
            ...
            ["-+|",hop_right_z_polarized_1],
            ["|-+",hop_right_z_polarized_2],
            ["+-|",hop_left_z_polarized_1],
            ["|+-",hop_left_z_polarized_2],
    		["n|n", int_list], # onsite interaction
    		]
    dynamic=[]
    H=hamiltonian(static,dynamic,dtype=np.complex128,basis=basis,check_symm=False,check_pcon = False)
    print("Hamiltonian constructed")
    return H#energies,vectors

    
def spectrum_con(t,t1,i_x,f_x,i_y,f_y,i_z,f_z,U,basis,Nx,Ny,N,Np,Raw_nos,state_no):
    
       
    H = hamil_cons(t,t1,i_x,f_x,i_y,f_y,i_z,f_z,U,basis,N)
    ...
    filename_energy = '%s/Energies_%d_sites_%d_particles_%0.2f_U.pkl' %(Raw_nos,N,Np,U)
    data_energy = en
    with open(filename_energy, 'wb') as outfile:
       pickle.dump(data_energy, outfile, pickle.HIGHEST_PROTOCOL)
       
    filename_states = '%s/Eigen_states_%d_x_sites_%d_y_sites_%d_particles_%0.2f_U.pkl' %(Raw_nos,Nx,Ny,Np,U)
...
def main(total,cmdargs):
    ...
    for i in range(N):
       ...
       if(i%2 == 0 and i%Nx<Nx-1):
         i_x.append(i)
         f_x.append(i+1)
    
       if(i%2 == 1 and int(i/Nx) <Ny-1):
         i_z.append(i)   
         f_z.append(i+Nx-1)  
         
       if(i%2 == 1 and i%Nx<Nx-2):
         i_y.append(i)
         f_y.append(i+1) 
# =============================================================================
    print(i_x,"i_x")
...
# =============================================================================
    ...
    spectrum_con(t,t1,i_x,f_x,i_y,f_y,i_z,f_z,u,basis,Nx,Ny,N,Np,Raw_nos,state_no)
